...
This server provides a safe, isolated environment to test and demonstrate the
React Server Components RCE vulnerability. It is locked to vulnerable versions
and should **never** be deployed to production or exposed to untrusted networks.
...
These versions contain the vulnerable Flight protocol deserializer that allows
remote code execution via prototype chain traversal.
...
Note: In production mode, error messages are sanitized. The `exploit-throw.sh`
method won't capture output, but `exploit-redirect.sh` still works.
...
```
docker build -t danielpacak/vulnerable-next-server:20-bullseye-slim .
```

```
docker run -it --rm -p 3000:3000 danielpacak/vulnerable-next-server:20-bullseye-slim
```
